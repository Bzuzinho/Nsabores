...
- STAFF/ADMIN: `/v1/admin/stock`, `stock/movements`, `suppliers`, `purchases`,
  `inventories`, `reseller-applications`, `business-accounts` e `price-lists`.

## Limitações

Não existem múltiplos armazéns, lotes, validade, EDI, conta corrente,
transportadora ou documentos fiscais. A matriz granular que reserva limites e
tabelas globais exclusivamente a `ADMIN` é o próximo endurecimento de permissões.
