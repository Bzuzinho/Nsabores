...
export default function ReturnOrderPage() {
  ...
  async function submit() {
    ...
    setError('');
    try {
      const result = await accountApi.post<ReturnResponse>(
        '/v1/account/returns',
        {
          orderId,
          resolution,
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : 'Não foi possível criar o pedido de devolução.',
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <section className="account-card">
      ...
      {order.items.map((item) => {
        const value = lines[item.id] ?? {
          quantity: 0,
          reason: '',
          declaredCondition: '',
        };
        const update = (patch: Partial<ReturnLine>) =>
          setLines((current) => ({
            ...current,
            [item.id]: { ...value, ...patch },
          }));
            ...
            {value.quantity > 0 && (
              <>
                ...
                <label>
                  Condição do artigo
                  <input
                    value={value.declaredCondition}
                    onChange={(event) =>
                      update({ declaredCondition: event.target.value })
                    }
                  />
                </label>
              </>
      ...
      <button
        className="button"
        disabled={saving || !['SHIPPED', 'DELIVERED'].includes(order.status)}
        onClick={() => void submit()}
      >
        {saving ? 'A submeter…' : 'Submeter pedido'}
      </button>
      {!['SHIPPED', 'DELIVERED'].includes(order.status) && (
        <p>Esta encomenda ainda não está elegível para devolução.</p>
      )}
    </section>
  );
}
