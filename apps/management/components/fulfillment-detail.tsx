...
export function ShipmentAdminDetail({ id }: { id: string }) {
  ...
  if (!shipment)
    return <div className="admin-state">{error || 'A carregar…'}</div>;
    ...
    <>
        ...
        <div>
          {!shipment.trackingNumber && (
            <button
              ...
              onClick={() => void action(`/v1/admin/shipments/${id}/label`)}
            >
              Criar etiqueta
            </button>
          )}
          {['READY', 'LABEL_CREATED'].includes(shipment.status) && (
            <button
              className="admin-primary"
              onClick={() => void action(`/v1/admin/shipments/${id}/dispatch`)}
            >
              Confirmar expedição
            </button>
          )}
      ...
      <section className="user-detail">
        ...
        <p>
          Expedida:{' '}
          {shipment.shippedAt
            ? new Date(shipment.shippedAt).toLocaleString('pt-PT')
            : '—'}
        </p>
        <p>
          Entrega prevista:{' '}
          {shipment.estimatedDeliveryAt
            ? new Date(shipment.estimatedDeliveryAt).toLocaleString('pt-PT')
            : '—'}
        </p>
        <p>
          Entregue:{' '}
        ...
        {shipment.events.map((event) => (
          <p key={event.id}>
            {new Date(event.occurredAt).toLocaleString('pt-PT')} — {event.code}:{' '}
            {event.description}
...
export function ReturnAdminDetail({ id }: { id: string }) {
  ...
  const reload = useCallback(async () => {
    try {
      const result = await managementApi.get<ReturnDetail>(
        `/v1/admin/returns/${id}`,
      );
      setRequest(result);
        ...
        Object.fromEntries(
          result.items.map((item) => [
            item.id,
            {
              ...
              eligibleRefundCents:
                item.eligibleRefundCents || item.unitPriceCents * item.quantity,
            },
          ]),
        ),
      );
  ...
  const refundTotal = useMemo(
    () =>
      Object.values(decisions).reduce(
        (sum, item) => sum + item.eligibleRefundCents,
        0,
      ),
    [decisions],
  );
  ...
  async function decide(approved: boolean) {
    if (
      !window.confirm(
        approved ? 'Aprovar esta devolução?' : 'Rejeitar esta devolução?',
      )
    )
      return;
    setError('');
    try {
      await managementApi.post(`/v1/admin/returns/${id}/decision`, {
        ...
        items: approved
          ? Object.entries(decisions).map(([returnItemId, item]) => ({
              returnItemId,
              ...item,
            }))
          : undefined,
  ...
  async function setStatus(status: string) {
    ...
    try {
      await managementApi.patch(`/v1/admin/returns/${id}/status`, {
        status,
      ...
      setError(
        reason instanceof Error ? reason.message : 'A alteração falhou.',
      );
    }
  }
    ...
    <>
      ...
      <section className="user-detail">
        ...
        {request.customerNotes && (
          <p>
            <strong>Notas do cliente:</strong> {request.customerNotes}
          </p>
        )}
        <h2>Artigos</h2>
          ...
          return (
            <div key={item.id}>
              <p>
                ...
                ({item.sku}) — comprado a {money(item.unitPriceCents)} / un.
              </p>
              <p>
                Motivo: {item.reason}
              ...
              {canDecide && decision && (
                <>
                  ...
                  <label>
                    Condição recebida
                    <input
                      value={decision.receivedCondition}
                      onChange={(event) =>
                        setDecisions((current) => ({
                          ...current,
                          [item.id]: {
                            ...decision,
                            receivedCondition: event.target.value,
                          },
                        }))
                      }
                    />
                  ...
                  <label>
                    ...
                    <input
                      ...
                      min="0"
                      step="0.01"
                      value={(decision.eligibleRefundCents / 100).toFixed(2)}
                      onChange={(event) =>
                        setDecisions((current) => ({
                          ...current,
                          [item.id]: {
                            ...decision,
                            eligibleRefundCents: Math.max(
                              0,
        ...
        <p>
          <strong>Total elegível: {money(refundTotal)}</strong>
        </p>
        <label>
          Notas internas
          <textarea
        ...
        {canDecide && (
          <div>
            <button className="admin-primary" onClick={() => void decide(true)}>
              Aprovar
            </button>
            <button onClick={() => void decide(false)}>Rejeitar</button>
          </div>
        )}
        {!canDecide && (
          <label>
            ...
            <select
              defaultValue=""
              onChange={(event) =>
                event.target.value && void setStatus(event.target.value)
              }
            >
...
export function SupportAdminDetail({ id }: { id: string }) {
  ...
  const reload = useCallback(async () => {
    try {
      const result = await managementApi.get<SupportDetail>(
        `/v1/admin/support-cases/${id}`,
      );
      setSupport(result);
  ...
  async function update(status: string) {
    ...
    try {
      await managementApi.patch(`/v1/admin/support-cases/${id}`, {
        status,
      ...
      setError(
        reason instanceof Error ? reason.message : 'A atualização falhou.',
      );
    }
  }
  ...
  async function addComment() {
    ...
    try {
      await managementApi.post(`/v1/admin/support-cases/${id}/comments`, {
        body: comment.trim(),
        isInternal: true,
      });
      setComment('');
  ...
  return (
    <>
      <header className="admin-header">
        <div>
          <p>
            <Link href="/apoio">← Pós-venda</Link>
          </p>
          <h1>{support.number}</h1>
          <p>
      ...
      <section className="user-detail">
        ...
        {support.orderId && (
          <p>
            Encomenda:{' '}
            <Link href={`/encomendas/${support.orderId}`}>
              {support.orderId}
            </Link>
          </p>
        )}
        {support.shipmentId && (
          <p>
            Expedição:{' '}
            <Link href={`/expedicoes/${support.shipmentId}`}>
              {support.shipmentId}
            </Link>
          </p>
        )}
        ...
        {support.comments.map((item) => (
          <p key={item.id}>
            {new Date(item.createdAt).toLocaleString('pt-PT')} —{' '}
            {item.firstName ? `${item.firstName} ${item.lastName ?? ''}: ` : ''}
            {item.body}
            {item.isInternal ? ' · interno' : ''}
          </p>
        ))}
        <label>
        ...
        <button className="admin-primary" onClick={() => void addComment()}>
