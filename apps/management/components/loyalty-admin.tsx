...
export function LoyaltyAdmin({
  ...
  async function load() {
    try {
      const [accountRows, ruleRows, cards] = await Promise.all([
        managementApi.get<LoyaltyAccountRow[]>(
          `/v1/admin/loyalty/accounts${search ? `?search=${encodeURIComponent(search)}` : ''}`,
        ),
  ...
  const metrics = useMemo(
    () => ({
      ...
      giftLiability: giftCards.reduce(
        (sum, row) => sum + row.balanceCents + row.reservedCents,
        0,
      ),
    }),
    [accounts, giftCards],
    ...
    try {
      await managementApi.post('/v1/admin/loyalty/rules', {
        ...
        minimumOrderCents: data.get('minimumOrderCents')
          ...
          : undefined,
        maximumPointsPerOrder: data.get('maximumPointsPerOrder')
  ...
  async function issueGiftCard(event: FormEvent<HTMLFormElement>) {
    ...
    const data = new FormData(form);
    try {
      const result = await managementApi.post<GiftCard & { code: string }>(
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : 'Não foi possível emitir o vale.',
      );
    } finally {
      setBusy(false);
    }
    ...
    <>
      ...
      {mode === 'dashboard' && (
        <>
          <section className="user-detail">
            ...
            <p>
              Responsabilidade em vales:{' '}
              <strong>{money(metrics.giftLiability)}</strong>
            </p>
            <p>
              <Link href="/fidelizacao/regras">Gerir regras</Link> ·{' '}
              <Link href="/vales-oferta">Gerir vales</Link>
            </p>
          </section>
          <section className="user-detail">
            <h2>Clientes</h2>
            ...
            {accounts.map((row) => (
              <article key={row.id}>
                ...
                <p>
                  Disponíveis: {row.availablePoints} · Pendentes:{' '}
                  {row.pendingPoints} · Reservados: {row.reservedPoints}
                </p>
                <Link href={`/fidelizacao/clientes/${row.userId}`}>
                  Ver ledger
                </Link>
              </article>
            ))}
        ...
        <>
          <section className="user-detail">
            ...
            <form className="auth-form" onSubmit={createRule}>
              ...
              <label>
                Multiplicador Clube (basis points)
                <input
                  name="clubMultiplierBasisPoints"
                  type="number"
                  min="0"
                  defaultValue="10000"
                  required
              ...
              <label>
                Compra mínima (cêntimos)
                <input name="minimumOrderCents" type="number" min="0" />
              </label>
              <label>
                Máximo por encomenda
                <input name="maximumPointsPerOrder" type="number" min="0" />
              </label>
              <label>
                Dias pendentes
                <input
      ...
      {mode === 'giftCards' && (
        <>
          <section className="user-detail">
            <h2>Emitir vale</h2>
            <form className="auth-form" onSubmit={issueGiftCard}>
              <label>
                Valor (cêntimos)
                <input
          ...
          <section className="user-detail">
            ...
            {giftCards.map((card) => (
              <article key={card.id}>
                <p>
                  <strong>•••• {card.codeLast4}</strong> · {card.status}
                </p>
                <p>
                  Saldo {money(card.balanceCents, card.currency)} · reservado{' '}
                  {money(card.reservedCents, card.currency)}
                </p>
