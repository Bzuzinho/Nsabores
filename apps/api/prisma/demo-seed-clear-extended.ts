...
async function main() {
  ...
  if (documentIds.length) {
    await db.fiscalDocumentEvent.deleteMany({ where: { documentId: { in: documentIds } } });
    await db.fiscalDocumentLine.deleteMany({ where: { documentId: { in: documentIds } } });
    await db.fiscalDocument.deleteMany({ where: { id: { in: documentIds } } });
  }
  await db.fiscalSeries.deleteMany({ where: { code: { startsWith: 'DEMO-' } } });
  ...
  if (giftCardIds.length) {
    await db.giftCardTransaction.deleteMany({ where: { giftCardId: { in: giftCardIds } } });
  ...
  if (loyaltyAccountIds.length) {
    await db.loyaltyTransaction.deleteMany({ where: { accountId: { in: loyaltyAccountIds } } });
    await db.loyaltyAccount.deleteMany({ where: { id: { in: loyaltyAccountIds } } });
  }
  ...
  if (subscriptionIds.length) {
    await db.clubSubscriptionCharge.deleteMany({ where: { subscriptionId: { in: subscriptionIds } } });
    await db.clubSubscriptionEvent.deleteMany({ where: { subscriptionId: { in: subscriptionIds } } });
    await db.clubSubscription.deleteMany({ where: { id: { in: subscriptionIds } } });
  }
  ...
  if (agreementIds.length) {
    await db.paymentContactEvent.deleteMany({ where: { agreementId: { in: agreementIds } } });
  ...
  if (orderIds.length) {
    ...
    if (supportCaseIds.length) {
      await db.supportCaseComment.deleteMany({ where: { supportCaseId: { in: supportCaseIds } } });
    ...
    if (returnIds.length) {
      await db.returnEvent.deleteMany({ where: { returnRequestId: { in: returnIds } } });
      await db.returnItem.deleteMany({ where: { returnRequestId: { in: returnIds } } });
    ...
    if (shipmentIds.length) {
      await db.shipmentEvent.deleteMany({ where: { shipmentId: { in: shipmentIds } } });
  ...
  if (productIds.length) {
    await db.productPersonalization.deleteMany({ where: { productId: { in: productIds } } });
    ...
    if (bundleIds.length) {
      await db.productBundleItem.deleteMany({ where: { bundleId: { in: bundleIds } } });
      await db.productBundleGroup.deleteMany({ where: { bundleId: { in: bundleIds } } });
  ...
  if (promotionIds.length) {
    await db.coupon.deleteMany({ where: { promotionId: { in: promotionIds } } });
    await db.promotionTarget.deleteMany({ where: { promotionId: { in: promotionIds } } });
  ...
  if (businessAccountIds.length) {
    await db.businessAccountUser.deleteMany({ where: { businessAccountId: { in: businessAccountIds } } });
    await db.businessAccount.deleteMany({ where: { id: { in: businessAccountIds } } });
  }
  ...
  if (priceListIds.length) {
    await db.priceListItem.deleteMany({ where: { priceListId: { in: priceListIds } } });
  ...
  if (inventoryIds.length) {
    await db.inventoryCountItem.deleteMany({ where: { inventoryCountId: { in: inventoryIds } } });
  ...
  if (receiptIds.length) {
    await db.purchaseReceiptItem.deleteMany({ where: { purchaseReceiptId: { in: receiptIds } } });
  ...
  if (purchaseIds.length) {
    await db.purchaseOrderItem.deleteMany({ where: { purchaseOrderId: { in: purchaseIds } } });
