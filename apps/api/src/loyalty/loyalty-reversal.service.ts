...
export class LoyaltyReversalService {
  ...
  refundPoints(
    ...
    if (points <= 0)
      throw new ConflictException('Os pontos a devolver têm de ser positivos.');
  ...
  refundGiftCard(
    ...
    if (amountCents <= 0)
      throw new ConflictException('O montante a devolver tem de ser positivo.');
    return this.prisma.$transaction(async (tx) => {
      const duplicate = await tx.$queryRaw<Array<Record<string, unknown>>>`
      ...
      if (balance + card.reservedCents > card.initialAmountCents) {
        throw new ConflictException(
          'A devolução excede o valor emitido do vale.',
        );
      }
      await tx.$executeRaw`
