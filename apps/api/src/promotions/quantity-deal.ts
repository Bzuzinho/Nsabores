...
export function calculateQuantityDeal(
  ...
  if (!Number.isInteger(unitPriceCents) || unitPriceCents < 0) {
    throw new RangeError(
      'O preço unitário tem de ser um inteiro não negativo.',
    );
  }
  if (!Number.isInteger(quantityBuy) || quantityBuy < 2) {
    throw new RangeError(
      'quantityBuy tem de ser um inteiro igual ou superior a 2.',
    );
  }
  if (
    !Number.isInteger(quantityPay) ||
    quantityPay < 1 ||
    quantityPay >= quantityBuy
  ) {
    throw new RangeError(
      'quantityPay tem de ser um inteiro entre 1 e quantityBuy - 1.',
    );
  }
